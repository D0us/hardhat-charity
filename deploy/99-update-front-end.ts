...
const updateUI: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
    ...
    if (process.env.UPDATE_FRONT_END && 0) {
        ...
        if (chainId in contractAddresses) {
            if (!contractAddresses[network.config.chainId!].includes(charity.address)) {
                contractAddresses[network.config.chainId!].push(charity.address)
            }
        } else {
            contractAddresses[network.config.chainId!] = [charity.address]
        }
